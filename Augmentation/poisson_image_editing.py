...
def test():
    dst_path = '../0-demo_images/poisson_dst.jpg'
    mask_path = '../0-demo_images/poisson_mask.png'
    src_path = '../0-demo_images/poisson_src.png'
    source = cv2.imread(src_path)
    target = cv2.imread(dst_path)
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    result = poisson_edit(source, mask, target)
    cv2.imshow("", result)
    cv2.waitKey()
